...
public class EllipsoidTests {
	...
	public void testIntersectionPoints() throws Exception {
	  ...
	  Ellipsoid ellipsoid = new Ellipsoid(new Point3D(0.0, 0.0, 0.0), 1, 2, 3);
	  Ellipsoid ellipsoid2 = new Ellipsoid(new Point3D(0.0, 0.0, -4), 1, 2, 3);
	  //Special Case Test: Tangent Line
	  Ellipsoid ellipsoid3 = new Ellipsoid(new Point3D(1, 0.0, -6), 1, 2, 3);
	  
	  Ellipsoid ellipsoid4 = new Ellipsoid(new Point3D(6, 0.0, -8), 5, 2, 3);
	  ...
	  System.out.println("Camera:\n " + camera);
	    for (int i = 0; i < HEIGHT; i++) {
		     for (int j = 0; j < WIDTH; j++) {
			      ...
			      List < Point3D > rayIntersectionPoints = ellipsoid.findIntersections(rays[i][j]);
			      List < Point3D > rayIntersectionPoints2 = ellipsoid2.findIntersections(rays[i][j]);
			      List < Point3D > rayIntersectionPoints3 = ellipsoid3.findIntersections(rays[i][j]);
			      List < Point3D > rayIntersectionPoints4 = ellipsoid4.findIntersections(rays[i][j]);
